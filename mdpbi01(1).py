...
d = []
for i in colonnes:
    d.append(m.addVar(vtype=GRB.INTEGER, lb=0, name="d%d" %(i+1)))
...
for i in lignes:
    ...
    m.addConstr(quicksum(c[i][j]*x[j] for j in colonnes) - z >= 0, "Controbj %d" % i)

for i in colonnes: 
    m.addConstr(x[i] - 2 * d[i] <= 0, "cm %d" % i)

m.addConstr(d[0] + d[1] <= 1, "dd %d" % i)
m.addConstr(d[2] + d[3] <= 1, "dd %d" % i)
